...
New Features REQUEST  CONTROL:
 --max-match, --mm\t\tNew features, Ignore matching :url require.
 --min-urls, --mu\t\tNew features, Minimize Access the  plugins :urls.
 --add-path, --ap\t\tNew features, Access Paths Like /favicon.ico,/robots.txt.
...
RISK and RISK_EXACT:
   The RISK level and RISK_EXACT controls the which risk level plugin will be loaded
  --risk-level, -r=RISK\tSets the plug-in level to invoke the specified risk level. Default: 1
    1. None\t\t\tInvoke None risk and higher level plug-ins for each target
    2. Low\t\t\tInvoke Low risk and higher level plug-ins for each target
    3. Mid\t\t\tInvoke Mid risk and higher level plug-ins for each target
    4. High\t\t\tInvoke High risk and higher level plug-ins for each target
  --risk-exact, --re\tNot Invoke higher level plug-ins. Default: false
...
opts = GetoptLong.new(
  ...
  ['--mm','--max-match', GetoptLong::NO_ARGUMENT],
  ['--mu','--min-urls', GetoptLong::NO_ARGUMENT],
  ['--ap','--add-path', GetoptLong::NO_ARGUMENT],
  ...
  ['-r', '--risk-level', GetoptLong::REQUIRED_ARGUMENT],
  ['--re', '--risk-exact', GetoptLong::NO_ARGUMENT],
...
begin
  opts.each do |opt, arg|
    ...
    when '-a', '--aggression'
      ...
      $AGGRESSION = arg.to_i
    when '-r', '--risk-level'
      raise "Risk level must be 1, 2, 3, 4. #{arg} is invalid." unless [1, 2, 3, 4].include? arg.to_i
      $RISK_LEVEL = arg.to_i
    when '--re', '--risk-exact'
      $RISK_EXACT = true
    when '-P', '--proxy'
