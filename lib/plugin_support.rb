...
class PluginSupport
  ...
  def self.load_plugins(list = nil)
    ...
    plugin_dirs.each do |d|
      ...
      if File.directory?(d)
        (Dir.glob("#{d}/**/**/*.rb") - minus_files).each { |x| PluginSupport.load_plugin(x) }  # 当前目录和更深子目录
      elsif File.exist?(d)
  ...
  def self.custom_plugin(c, *option)
    if option == ['grep']
      ...
      # then it's ok..
      if c =~ /:(text|ghdb|md5|mmh3|allhash|regexp|tagpattern)=>[\/'"].*/
        matches = "matches([\{#{c}\}])"
      end
